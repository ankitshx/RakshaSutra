    ...
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="space-y-2 text-center sm:text-left">
        ...
        <p className="text-sm text-slate-400 max-w-3xl">
          Aggregated threat telemetry, real-time cyber attacks feed, live provider health, and global IOC repository indexed across URLhaus, VirusTotal, AbuseIPDB, and local heuristic engines.
        </p>
      </div>

      {/* Main Grid: Left Side = Live Global Attacks Stream, Right Side = Provider Matrix & IOC Search */}
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {providers.map((p) => (
                <div
                  key={p.name}
                  className="p-4 rounded-2xl bg-slate-900/80 border border-cyber-border backdrop-blur-md space-y-2 shadow-lg"
                >
                  <div className="flex items-center justify-between">
                    <h4 className="text-xs font-bold text-white font-mono">{p.display_name}</h4>
                    <span
          ...
          <div className="p-6 rounded-2xl bg-slate-900/80 border border-cyber-border backdrop-blur-xl shadow-xl space-y-4">
            ...
            <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-2">
              <input
                ...
                className="flex-1 px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-xs sm:text-sm text-white placeholder-slate-500 font-mono focus:outline-none focus:border-cyan-500"
              />
              <button
                type="submit"
                disabled={isSearching}
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-300 font-mono flex items-center gap-2">
            <Database className="w-3.5 h-3.5 text-cyan-400" /> Active Global Threat Feed Database ({feed.length})
          </h3>
            ...
            <tbody className="divide-y divide-slate-800/60">
              {feed.map((item) => (
                <tr key={item.id} className="hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="p-3.5">
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-rose-950/80 text-rose-400 border border-rose-500/40">
                      {item.threat_category}
                    </span>
                  </td>
                  <td className="p-3.5 text-cyan-400">{item.confidence}%</td>
                  <td className="p-3.5 text-slate-400">{item.source}</td>
                  <td className="p-3.5 text-slate-300 max-w-[250px] truncate">{item.description}</td>
                </tr>
              ))}
            </tbody>
          </table>
