...
from app.services.ai_sentinel_service import (
    generate_ai_sentinel_audit,
...
@router.get("/ai-sentinel/audit")
def get_ai_sentinel_audit(
    current_admin: User = Depends(get_current_admin),
    db: Session = Depends(get_db)
):
    """
    Super Admin & SOC AI Sentinel Audit.
    Scans live telemetry, database records, threat telemetry, and security posture.
    Generates intelligent audits of immediate updates needed and evolutionary feature recommendations.
    """
    audit = generate_ai_sentinel_audit(db)
    return audit

@router.post("/ai-sentinel/dispatch-telegram")
async def dispatch_ai_sentinel_to_telegram(
    req: TelegramDispatchRequest = TelegramDispatchRequest(),
    current_admin: User = Depends(get_current_admin),
    db: Session = Depends(get_db)
):
    """
    Dispatches the latest AI Sentinel Audit directly to Telegram.
    Can use environment variables (TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID)
    or custom credentials supplied by the Super Admin in the request.
    """
    audit = generate_ai_sentinel_audit(db)
    message = format_telegram_report(audit)

    if req.custom_note:
        message += f"\n\n📝 *Admin Note:* {req.custom_note}"

    result = await dispatch_telegram_alert(
        message=message,
    ...
    if not result.get("success"):
        raise HTTPException(
            status_code=400,
            detail=result.get("error", "Failed to dispatch Telegram message.")
        )

    return {
        "success": True,
        "message": "AI Sentinel intelligence report successfully dispatched to Telegram.",
        "telegram_response": result
    }

@router.get("/ai-sentinel/config")
def get_ai_sentinel_config(
    current_admin: User = Depends(get_current_admin)
):
    """
    Checks if Telegram alerting is configured via environment variables.
    """
    token = settings.TELEGRAM_BOT_TOKEN or ""
    chat_id = settings.TELEGRAM_CHAT_ID or ""

    masked_token = (token[:6] + "..." + token[-4:]) if len(token) > 10 else ("Configured" if token else "")
    masked_chat = (chat_id[:3] + "..." + chat_id[-2:]) if len(chat_id) > 5 else ("Configured" if chat_id else "")

    return {
        "is_configured": bool(token and chat_id),
        "alerts_enabled": settings.TELEGRAM_ALERTS_ENABLED,
        "bot_token_preview": masked_token,
        "chat_id_preview": masked_chat
    }
